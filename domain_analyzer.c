...
{
    // cauta simbolul cu numele dat in vextorul symbols , si returneaza un pointer catre el sau null 
    // cautam de la dreapta la stanga  , in caz ca a fost redefinit sa gasim ultima definitie
...
{ 
   //basically the same ca si findSymbol dar cauta doar in current scope
    ...
    if(typeBase(&t)) 
     {
      ...
      if(consume(ID))  
      {
        ...
        if(arrayDecl(&t))
        {
            if(t.nElements==0)
            {
                tkerr(crtTk,"a vector variable must have a specific dimesnion");
            }
        }
...
int structDef()  //TREBUIE MODIFICAT
{
    if(!consume(STRUCT)) 
     return 0;
